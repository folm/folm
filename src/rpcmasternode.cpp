...
UniValue obfuscation(const UniValue& params, bool fHelp)
{
    ...
    if(params[0].get_str() == "start"){
        ...
        bool result = obfuScationPool.DoAutomaticDenominating();
//        fEnableObfuscation = result;
        return "Mixing " + (result ? "started successfully" : ("start failed: " + obfuScationPool.GetStatus() + ", will retry"));
    }
    ...
    if(params[0].get_str() == "status"){
        return "Mixing status: " + obfuScationPool.GetStatus();
    }

    return "Unknown command, please see \"help obfuscation\"";
}
...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start local Hot masternode configured in folm.conf\n"
