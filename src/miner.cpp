        ...
        while (mi != mempool.mapTx.get<3>().end() || !clearedTxs.empty()) {
            ...
            if (fPrintPriority) {
                ...
                LogPrintf("priority %.1f fee %s txid %s\n",
                          Priority , CFeeRate(iter->GetModifiedFee(), nTxSize).ToString(), tx.GetHash().ToString());
            }
...
void BitcoinMiner(const CChainParams& chainparams, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        auto_ptr<CBlockTemplate> pblocktemplate(CreateNewBlock(chainparams, coinbaseScript->reserveScript, pwallet, fProofOfStake));
        if (!pblocktemplate.get())
            continue;
...
void GenerateBitcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreadBitcoinMiner, boost::cref(chainparams)));
}
