...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, CAmount nFees)
{
    ...
    CAmount blockValue = GetBlockSubsidy(chainActive.Tip()->nBits, chainActive.Tip()->nHeight, Params().GetConsensus());
    ...
    if (hasPayment && chainActive.Tip()->nHeight > 10800) {
        txNew.vout.resize(2);

        txNew.vout[1].scriptPubKey = payee;
        txNew.vout[1].nValue = masternodePayment + (nFees/2);

        txNew.vout[0].nValue -= masternodePayment  + (nFees/2);
