...
/* *** BEGIN OBFUSCATION MAGIC - FOLM **********
    Copyright (c) 2014-2015, Folm Developers
        eduffield - evan@folm.org
        udjinm6   - udjinm6@folm.org
*/
...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    } else if (strCommand == NetMsgType::DSSTATUSUPDATE) { //Obfuscation Complete
...
bool CObfuscationPool::IsCollateralValid(const CTransaction& txCollateral){
    ...
    BOOST_FOREACH(const CTxOut o, txCollateral.vout){
        ...
        if(!o.scriptPubKey.IsNormalPaymentScript()){
            LogPrintf ("CObfuscationPool::IsCollateralValid - Invalid Script %s", txCollateral.ToString());
            return false;
        }
    }
    ...
    if(missingTx){
        LogPrint("obfuscation", "CObfuscationPool::IsCollateralValid - Unknown inputs in collateral transaction - %s", txCollateral.ToString());
        return false;
    }
    ...
    LogPrint("obfuscation", "CObfuscationPool::IsCollateralValid %s", txCollateral.ToString());
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if(!fObfuscationMultiSession && pCurrentBlockIndex->nHeight - cachedLastSuccess < minBlockSpacing) {
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeFolmAmount*COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if(!sessionFoundMasternode){
        ...
        if(!fObfuscationMultiSession && pwalletMain->GetDenominatedBalance(true) > 0) { //get denominated unconfirmed inputs
...
bool CObfuscationPool::CreateDenominated(CAmount nTotalValue)
{
    ...
    BOOST_REVERSE_FOREACH(CAmount v, obfuScationDenominations){
        ...
        if (v != obfuScationDenominations[0] && pwalletMain->CountInputsWithAmount(v) > DENOMS_COUNT_MAX){
            strAutoDenomResult = strprintf(_("Too many %f denominations, removing."), (float)v/COIN);
            LogPrintf("DoAutomaticDenominating : %s\n", strAutoDenomResult);
            obfuScationDenominationsSkipped.push_back(v);
            continue;
        }
...
void CObfuscationPool::UpdatedBlockTip(const CBlockIndex *pindex)
{
    pCurrentBlockIndex = pindex;
    LogPrint("obfuscation", "pCurrentBlockIndex->nHeight: %d\n", pCurrentBlockIndex->nHeight);
...
void ThreadCheckObfuScationPool()
{
    ...
    // Make this thread recognisable as the Obfuscation/Masternode thread
    RenameThread("folm-obfuscation");
