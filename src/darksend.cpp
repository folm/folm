...
std::vector<CFolmsendQueue> vecFolmsendQueue;
/** Keep track of the used masternodes */
std::vector<CTxIn> vecMasternodesUsed;
// keep track of the scanning errors I've seen
map<uint256, CFolmsendBroadcastTx> mapFolmsendBroadcastTxes;
//
...
void ProcessFolmsend(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, bool &isFolmsend)
{
    ...
    if (strCommand == "dsf") { //DarkSend Final tx
        ...
        if(darkSendPool.sessionUsers == 0) {
            if(vecMasternodes[mn].nLastDsq != 0 &&
                ...
                std::string strError = _("Last Folmsend was too recent.");
                pfrom->PushMessage("dssu", darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), MASTERNODE_REJECTED, strError);
                return;
            }
        }
        ...
        if(dsq.ready) {
            ...
            darkSendPool.PrepareFolmsendDenominate();
        } else {
            BOOST_FOREACH(CFolmsendQueue q, vecFolmsendQueue){
            ...
            vecFolmsendQueue.push_back(dsq);
            dsq.Relay();
            dsq.time = GetTime();
        }
		...
		if(GetTransaction(i.prevout.hash, tx2, hash)){
            ...
            if (nValueIn > DARKSEND_POOL_MAX) {
                ...
                error = _("Value more than Folmsend pool maximum allows.");
                pfrom->PushMessage("dssu", darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), MASTERNODE_REJECTED, error);
                return;
            }
...
int GetInputFolmsendRounds(CTxIn in, int rounds)
{
    ...
    if (wtx != NULL) {
        ...
        BOOST_FOREACH(CTxIn in2, wtx->vin)
        {
            if(pwalletMain->IsMine(in2))
            {
                ...
                int n = GetInputFolmsendRounds(in2, rounds+1);
                if(n != -3) return n;
            }
        }
    }
...
//
// Unlock coins after Folmsend fails or succeeds
//
...
//
// Check the Folmsend progress and send client updates if a masternode
//
    ...
    if(state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        ...
        LOCK2(cs_main, pwalletMain->cs_wallet);
        {
            if (fMasterNode) { //only the main node is master atm
                ...
                if(!mapFolmsendBroadcastTxes.count(txNew.GetHash())){
                    CFolmsendBroadcastTx dstx;
                    dstx.tx = txNew;
                    dstx.vin = activeMasternode.vin;
                    dstx.vchSig = vchSig;
                    dstx.sigTime = sigTime;

                    mapFolmsendBroadcastTxes.insert(make_pair(txNew.GetHash(), dstx));
                }
...
//
// Why bother? Folmsend uses collateral to ensure abuse to the process is kept to a minimum.
...
//  - Folmsend is completely free, to pay miners we randomly pay the collateral of users.
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.002Folm per transaction on average.
            */
            if(r <= 20)
            {
...
void CDarkSendPool::CheckTimeout(){
    if(!fEnableFolmsend && !fMasterNode) return;
    ...
    int c = 0;
    vector<CFolmsendQueue>::iterator it;
    for(it=vecFolmsendQueue.begin();it<vecFolmsendQueue.end();it++){
        if((*it).IsExpired()){
            if(fDebug) LogPrintf("CDarkSendPool::CheckTimeout() : Removing expired queue entry - %d\n", c);
            vecFolmsendQueue.erase(it);
            break;
        }
        c++;
    }
...
//
void CDarkSendPool::SendFolmsendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount){
    if(darkSendPool.txCollateral == CMutableTransaction()){
        LogPrintf ("CFolmsendPool:SendFolmsendDenominate() - Folmsend collateral not set");
        return;
    }
    ...
    if(!sessionFoundMasternode){
        LogPrintf("CDarkSendPool::SendFolmsendDenominate() - No masternode has been selected yet.\n");
        UnlockCoins();
        SetNull(true);
        return;
    }
    ...
    if(fMasterNode) {
        LogPrintf("CDarkSendPool::SendFolmsendDenominate() - DarkSend from a masternode is not supported currently.\n");
        return;
    }

    UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);

    LogPrintf("CDarkSendPool::SendFolmsendDenominate() - Added transaction to pool.\n");
...
// Folmsend transaction was completed (failed or successed)
...
//
// Passively run Folmsend in the background to anonymize funds based on the given configuration.
//
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    if(!fEnableFolmsend) {
        if(fDebug) LogPrintf("CDarkSendPool::DoAutomaticDenominating - Folmsend is disabled\n");
        strAutoDenomResult = _("Folmsend is disabled.");
        return false;
    }
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeFolmAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fFolmSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Folmsend for %f Folm CENT - sessionTotalValue %d\n", fFolmSubmitted, sessionTotalValue);
        ...
        if(nUseQueue > 33){
            ...
            BOOST_FOREACH(CFolmsendQueue& dsq, vecFolmsendQueue){
                CService addr;
...
bool CDarkSendPool::PrepareFolmsendDenominate()
{
    // Submit transaction to the pool if we get here, use sessionDenom so we use the same amount of money
    std::string strError = pwalletMain->PrepareFolmsendDenominate(0, nDarksendRounds);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100Folm+1 ( bit on if present )
    // bit 1 - 10Folm+1
    // bit 2 - 1Folm+1
    // bit 3 - .1Folm+1
    // bit 3 - non-denom
...
bool CFolmsendQueue::Sign()
{
    ...
    {
        LogPrintf("CFolmsendQueue():Relay - ERROR: Invalid masternodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if(!darkSendSigner.SignMessage(strMessage, errorMessage, vchSig, key2)) {
        LogPrintf("CFolmsendQueue():Relay - Sign message failed");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(pubkey2, vchSig, strMessage, errorMessage)) {
        LogPrintf("CFolmsendQueue():Relay - Verify message failed");
        return false;
    }
...
bool CFolmsendQueue::CheckSignature()
{
    BOOST_FOREACH(CMasterNode& mn, vecMasternodes) {
        ...
        if(mn.vin == vin) {
            ...
            if(!darkSendSigner.VerifyMessage(mn.pubkey2, vchSig, strMessage, errorMessage)){
                return error("CFolmsendQueue::CheckSignature() - Got bad masternode address signature %s \n", vin.ToString().c_str());
            }
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeFolmAmount = (rand() % nLeftToAnon)+3;
                } else {
