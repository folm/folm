...
UniValue mnbudget(const UniValue& params, bool fHelp)
{
    ...
    {
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare <proposal-name> <url> <payment-count> <block-start> <folm-address> <monthly-payment-folm>'");
    ...
    {
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit <proposal-name> <url> <payment-count> <block-start> <folm-address> <monthly-payment-folm> <fee-tx>'");
