...
class CFolmsendQueue;
class CFolmsendBroadcastTx;
...
extern std::vector<CFolmsendQueue> vecFolmsendQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CFolmsendBroadcastTx> mapFolmsendBroadcastTxes;
extern CActiveMasternode activeMasternode;

//specific messages for the Folmsend protocol
...
int GetInputFolmsendRounds(CTxIn in, int rounds=0);
...
public:
    ...
    // Passively run Folmsend in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun=false, bool ready=false);
    bool PrepareFolmsendDenominate();


    // check for process in Folmsend
    void Check();
    ...
    void SendFolmsendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
