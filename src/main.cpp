...
const string strMessageMagic = "Folm Signed Message:\n";
    ...
    for (const CTxOut &txout : tx.vout) {
        ...
        if (txout.nValue < 0)
            return state.DoS(100, error("%s: tx.vout[%d].nValue negative (%s, empty=%s, coinstake=%s)", __func__, i,
                                        txout.ToString(), (txout.IsEmpty() ? "yes" : "no"),
                                        (tx.IsCoinStake() ? "yes" : "no")),
...
bool AcceptToMemoryPool(CTxMemPool &pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,
...

    {
        ...
        if (nSigOps > MAX_TX_SIGOPS)
            return state.DoS(0, error("AcceptToMemoryPool : too many sigops %s, %d > %d", hash.ToString(), nSigOps,
                                      MAX_TX_SIGOPS),
...
CAmount GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount) {
    ...
    if (nHeight < Params().RAMP_TO_BLOCK()) {
        ...
        ret = blockValue / 18 * 15;
    } else if (nHeight <= 525600 && nHeight > 262800) {
        ret = blockValue / 24 * 20;
    } else if (nHeight <= 788400 && nHeight > 525600) {
        ret = blockValue / 18 * 15;
    } else if (nHeight <= 1051200 && nHeight > 788400) {
        ret = blockValue / 12 * 10;
    } else if (nHeight <= 1314000 && nHeight > 1051200) {
        ret = blockValue / 9 * 7.5;
    } else if (nHeight <= 1576800 && nHeight > 1314000) {
        ret = blockValue / 6 * 5;
    } else if (nHeight <= 1839600 && nHeight > 1576800) {
        ret = blockValue / 5 * 4;
...
bool IsInitialBlockDownload() {
    ...
    bool state = (chainActive.Height() < chainActive.Tip()->nHeight - 24 * 6 ||
                  ...
                  GetTime() - 8 * 60 * 60); // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
    ...
    if (block.IsProofOfWork()) {
        ...
        if (!IsInitialBlockDownload() && !IsBlockValueValid(block, nReward)) {
            return state.DoS(100,
                             error("%s: reward pays too much (actual=%d vs limit=%d) (nHeight=%d, nFees=%d)", __func__,
...
bool ProcessNewBlock(CValidationState &state, CNode *pfrom, CBlock *pblock, CDiskBlockPos *dbp) {
    ...
    if (pblock->IsProofOfStake() && stake->setStakeSeen.count(pblock->GetProofOfStake()) &&
...
bool static LoadBlockIndexDB() {
    ...
    if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false) &&
        ...
        LogPrintf("%s: lastIndexPos=%d blockFileSize=%d\n", __func__,
                  vSortedByHeight[vSortedByHeight.size() - 1].second->GetBlockPos().nPos,
        ...
        LogPrintf("%s: Last block properly recorded: #%d %s\n", __func__, pindexLastMeta->nHeight,
                  pindexLastMeta->GetBlockHash().GetHex());
        ...
        vinfoBlockFile[nLastBlockFile].nSize =
                pindexLast->GetBlockPos().nPos + ::GetSerializeSize(lastBlock, SER_DISK, CLIENT_VERSION);;
        ...
        if (!isFixed) {
            strError = "Failed reading from database. " + strError +
                       ". The block database is in an inconsistent state and may cause issues in the future."
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    LogPrint("net", "getblocks (%d) %s to peer=%d\n", chainActive.Tip()->nHeight, inv.hash.ToString(),
                             pfrom->id);
                }
            }
...
#       else
        if (!processed) ProcessFolmsend(pfrom, strCommand, vRecv, processed);
...
bool SendMessages(CNode *pto, bool fSendTrickle) {
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && fFetch /*&& !fImporting*/ && !fReindex) {
            ...
            if (nSyncStarted == 0 || chainActive.Tip()->GetBlockTime() >
